...
public class MaterialStockMovementDao {
    ...
    public List<MaterialStockMovement> findAll() {
        try { Session ss = HibernateUtil.getSessionFactory().openSession(); Query q = ss.createQuery("FROM MaterialStockMovement ORDER BY movementDate DESC, createdAt DESC"); List<MaterialStockMovement> list = q.list(); ss.close(); return list; } catch (Exception e) { e.printStackTrace(); } return Collections.EMPTY_LIST;
    }
    public List<MaterialStockMovement> findByProject(String projectId) {
        try { Session ss = HibernateUtil.getSessionFactory().openSession(); Query q = ss.createQuery("FROM MaterialStockMovement WHERE project.id = :projectId ORDER BY movementDate DESC, createdAt DESC"); q.setParameter("projectId", projectId); List<MaterialStockMovement> list = q.list(); ss.close(); return list; } catch (Exception e) { e.printStackTrace(); } return Collections.EMPTY_LIST;
    }
